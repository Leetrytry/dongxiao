...
- 曲目练习：只使用默认本地图片谱曲库，支持图片谱 OCR 转动态谱、原图动态高亮和谱面驱动的智能背景伴奏。
...
1. 在首页选择 `曲目练习`。
2. 在 `曲目选择` 中选择默认曲库自带的 `图片转谱 · ...` 曲目。
3. 点击 `播放伴奏`，App 会播放由谱面驱动的智能背景伴奏，并同步高亮动态谱面。
4. `曲目选择` 中的 `图片转谱 · ...` 曲目来自本地图片谱 OCR 转换，播放时会在原图片谱上动态高亮当前音符。
5. 可以在 `本地图片谱` 中选择内置图片谱；选择图片谱会自动切换到对应的 `图片转谱 · ...` 动态曲目，多页曲谱可用 `上一页`、`下一页` 翻页查看。
6. 跟随当前高亮音符练习，点击 `停止伴奏` 可中断播放。
...
- 曲目练习只支持默认本地图片谱曲库，不再提供 ABC URL 导入、在线开源曲库或第三方曲目添加入口。
