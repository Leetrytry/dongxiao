...
- 曲目练习：内置短曲，支持合成伴奏播放、简谱数字动态谱子，以及从 HTTPS ABC 曲谱链接导入第三方曲目。
...
1. 在首页选择 `曲目练习`。
2. 选择内置曲目，例如 `小星星片段`。
3. 点击 `播放伴奏`，App 会播放合成伴奏并同步高亮简谱数字动态谱子。
4. 也可以输入可直接下载 ABC 文本的 HTTPS 链接，点击 `导入` 后追加到曲目列表。
5. 跟随当前高亮音符练习，点击 `停止伴奏` 可中断播放。
...
- 曲目练习目前使用内置短曲、本地合成伴奏和 ABC 文本导入。导入功能需要第三方链接直接返回标准 ABC 文本；复杂装饰音、复声部、多歌词等高级 ABC 语法只做基础兼容。
