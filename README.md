...
- 曲目练习：内置短曲，支持合成伴奏播放、简谱式动态谱面、默认本地图片谱库、App 内开源曲库访问，以及从 HTTPS ABC 曲谱链接导入第三方曲目并保存到本地。
...
7. 可以在 `本地图片谱` 中选择内置图片谱；多页曲谱可用 `上一页`、`下一页` 翻页查看。
8. 跟随当前高亮音符练习，点击 `停止伴奏` 可中断播放。
...
- 默认本地图片谱库使用 `app/src/main/assets/score_images/` 中的图片资源，只负责查阅曲谱；这些图片谱暂未转换成可播放伴奏的结构化动态简谱。
