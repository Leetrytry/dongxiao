...
- 曲目练习：内置短曲，支持合成伴奏播放、简谱式动态谱面、默认本地图片谱库、图片谱 OCR 转动态谱、App 内开源曲库访问，以及从 HTTPS ABC 曲谱链接导入第三方曲目并保存到本地。
...
7. `曲目选择` 中的 `图片转谱 · ...` 曲目来自本地图片谱 OCR 转换，可作为动态简谱播放。
...
- 默认本地图片谱库使用 `app/src/main/assets/score_images/` 中的图片资源，同时使用 `app/src/main/assets/jianpu_text/` 中的 OCR 文本生成 `图片转谱 · ...` 动态曲目。OCR 转谱可能存在错音、漏音和节奏误差，后续应以原图片谱为准逐首校对。
