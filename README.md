...
- 曲目练习：内置短曲，支持智能背景伴奏、简谱式动态谱面、默认本地图片谱库、图片谱 OCR 转动态谱、原图动态高亮、App 内开源曲库访问，以及从 HTTPS ABC 曲谱链接导入第三方曲目并保存到本地。
...
1. 在首页选择 `曲目练习`。
2. 选择内置曲目，例如 `小星星片段`。
3. 点击 `播放伴奏`，App 会播放由谱面驱动的智能背景伴奏，并同步高亮动态谱面。
...
当前版本先使用谱面驱动的智能背景伴奏，未直接打包第三方原版伴奏。已检索到的可人工确认资源：

- `G弦上的咏叹调`：BitMidi 提供 `Bach_Air_on_G_string_BWV1068.mid` 下载页，适合作为后续 MIDI 伴奏替换候选；使用前需确认再分发许可。
- `一步之遥 / Por Una Cabeza`：IMSLP 有曲目页面和历史录音条目，mfiles 提供 violin/piano 改编版 MP3/MIDI 下载；使用前需确认所在地区公版状态及改编/录音许可。
...
- 曲目练习目前使用内置短曲、本地智能背景伴奏、在线 ABC 曲库和 ABC 文本导入。导入功能需要第三方链接直接返回标准 ABC 文本；复杂装饰音、复声部、多歌词等高级 ABC 语法只做基础兼容。
- 网上可找到部分曲目的 MIDI/MP3 资源，例如巴赫《G弦上的咏叹调》和《一步之遥》存在公开 MIDI/音频页面；现代流行歌曲原版伴奏通常受版权保护，当前版本不直接打包第三方原版伴奏。后续若提供授权音频，可在现有动态谱时间轴基础上逐首做音频替换和校准。
